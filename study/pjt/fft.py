...
from test.image import Image, Function
...
def log10(x):
    return Log10()(x).set_name(x.name)

def log1p(x):
    return Log1p()(x).set_name(x.name)
...
def fft(x):
    return FFT()(x).set_name(x.name).add_name("fft")

def fftshift(x):
    return FFTShift()(x).set_name(x.name).add_name("fftshift")
...
def dct(x):
    return DCT()(x).set_name(x.name).add_name("dct")
